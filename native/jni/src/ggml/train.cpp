...
int64_t get_example_targets_batch(
    ...
    // AKLOGI("%s: example_id=%d n_batch=%d n_train_samples=%zu\n", __func__, example_id, n_batch, n_train_samples);
    for (int k=0; k<n_batch; ++k) {
        ...
        // AKLOGI("%s: sample_idx=%zu sample=%zu\n", __func__, sample_idx, sample);
...
void print_duration(double fmillis) {
    ...
    if (days > 0) {
        AKLOGI("%lldd ", (long long int) days);
    }
    AKLOGI("%02lld:%02lld:%02lld", (long long int) hours, (long long int) minutes, (long long int) seconds);
}
...
size_t tokenize_file(
    ...
    if (f.size == 0) {
        ...
        AKLOGI("%s: warning: empty or not existing training data file '%s'\n",
               __func__, filename);
    ...
    if (sample_start.size() == 0) {
        ...
        if (out_samples_begin.size() == 0) {
            AKLOGI("%s: warning: sample start pattern '%s' not found. inserting single sample at data begin\n",
        ...
        if (found_too_big_sample > 0) {
            AKLOGI("%s: warning: found %zu samples (max length %zu) that exceed context length of %u. samples will be cut off.\n",
                   __func__, found_too_big_sample, found_max_sample_size, context_length);
        }

        if (found_too_small_sample > 0) {
            AKLOGI("%s: warning: found %zu samples (min length %zu) that are shorter than context length of %u.\n",
                   __func__, found_too_small_sample, found_min_sample_size, context_length);
        }

        if (found_empty_sample) {
            AKLOGI("%s: warning: found %zu empty samples.\n",
                   __func__, found_empty_sample);
        }
    }
    AKLOGI("%s: total number of samples: %zu\n",
...
void train_opt_callback(void * vdata, int accum_step, float * sched, bool * cancel) {
    ...
    if (train->shuffle_next_sample >= train->shuffle_sample_count) {
        ++train->train_epochs;
        AKLOGI("%s: reshuffle samples. completed epochs: %llu\n", __func__, (long long unsigned) train->train_epochs);
