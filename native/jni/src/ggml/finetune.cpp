...
int finetune_train(struct train_params params) {
    ...
    print_lora_params(&lora.hparams);
    AKLOGI("%s: total train_iterations %llu\n", __func__, (long long unsigned) train->train_its);
    AKLOGI("%s: seen train_samples     %llu\n", __func__, (long long unsigned) train->train_samples);
    AKLOGI("%s: seen train_tokens      %llu\n", __func__, (long long unsigned) train->train_tokens);
    AKLOGI("%s: completed train_epochs %llu\n", __func__, (long long unsigned) train->train_epochs);
    AKLOGI("%s: lora_size = %zu bytes (%.1f MB)\n", __func__, (ggml_used_mem(lora.ctx) + lora.data.size()), (float) (ggml_used_mem(lora.ctx) + lora.data.size()) / (1024.0f*1024.0f));
    ...
    AKLOGI("%s: opt_size  = %zu bytes (%.1f MB)\n", __func__, ggml_get_mem_size(opt->ctx), (float) ggml_get_mem_size(opt->ctx) / (1024.0f*1024.0f));
    AKLOGI("%s: opt iter %d\n", __func__, opt->iter);
    ...
    size_t max_input_size = GGML_PAD(ggml_nbytes(tokens_input), tensor_alignment) +
                            ...
                            tensor_alignment;
    AKLOGI("%s: input_size = %zu bytes (%.1f MB)\n", __func__, max_input_size, (float) max_input_size / (1024.0f*1024.0f));
    ...
    AKLOGI("%s: compute_size = %zu bytes (%.1f MB)\n", __func__, max_compute_size, (float) max_compute_size / (1024.0f*1024.0f));
    AKLOGI("%s: evaluation order = %s\n", __func__,
    ...
    for(std::vector<llama_token> example : params.training_data) {
        train_samples_begin.push_back(train_tokens.size());
        train_samples_size.push_back(example.size());
        train_tokens.insert(train_tokens.end(), example.begin(), example.end());
    }
    GGML_ASSERT(train_samples_begin.size() == train_samples_size.size());

    AKLOGI("%s: number of training tokens: %zu\n", __func__, train_tokens.size());
    ...
    AKLOGI("%s: number of unique tokens: %d\n", __func__, n_unique_tokens);
    ...
    const bool changed_train_data = true;// || (shuffle_samples_hash != train->shuffle_samples_hash) || (train->shuffle_sample_count != train_samples_size.size());
    if (changed_train_data) {
        AKLOGI("%s: train data seems to have changed. restarting shuffled epoch.\n", __func__);
    }
    if (params.common.force_reshuffle) {
        AKLOGI("%s: forced reshuffling of data. restarting with newly shuffled epoch.\n", __func__);
    }
    ...
    AKLOGI("%s: work_size = %zu bytes (%.1f MB)\n", __func__, max_work_size, (float) max_work_size / (1024.0f*1024.0f));
    ...
    AKLOGI("%s: total training time: ", __func__);
